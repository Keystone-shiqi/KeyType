...
struct PermissionGuideView: View {
    ...
    var body: some View {
        ...
        // from System Settings *without* KeyType becoming active (user
    ...
    private var content: some View {
        VStack(spacing: 16) {
            ...
            Text(L(
                "请授权以下权限,以允许 KeyType 使用你的麦克风并监听快捷键和完成输入",
                "Please grant the permissions below so KeyType can use your microphone and listen for hotkeys to type for you."
            ))
    ...
    private var launchButton: some View {
        Button(action: dismissGuide) {
            Text(L("启动 KeyType", "Launch KeyType"))
    ...
    /// silently parking the app in the menu bar), and bring KeyType to the
    /// front.
