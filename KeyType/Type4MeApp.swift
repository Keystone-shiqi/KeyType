...
struct KeyTypeApp: App {
    ...
    var body: some Scene {
        ...
        Window(L("KeyType 设置", "KeyType Settings"), id: "settings") {
            SettingsView()
                .environment(appDelegate.appState)
                .environment(appDelegate.appUpdater)
        }
        ...
        Window(L("KeyType 设置向导", "KeyType Setup"), id: "setup") {
            SetupWizardView()
                .environment(appDelegate.appState)
                .environment(appDelegate.appUpdater)
        ...
        Window(L("KeyType 授权引导", "KeyType Permissions"), id: "permission-guide") {
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        NSLog("[KeyType] applicationDidFinishLaunching")
        ...
        Task {
            await session.setOnASREvent { event in
                Task { @MainActor in
                    switch event {
                    case .ready:
                        NSLog("[KeyType] ready event received")
                        ...
                        Task { @MainActor in
                            ...
                            NSLog("[KeyType] playStart firing")
    ...
    private func registerHotkeys(for provider: ASRProvider) {
            ...
            return ModeBinding(
                ...
                onStart: { [weak self] in
                    ...
                    if phase == .recording || phase == .preparing {
                        NSLog("[KeyType] >>> HOTKEY: toggle desync – onStart while recording, redirecting to STOP (phase=%@)", String(describing: phase))
                    ...
                    if phase == .processing {
                        NSLog("[KeyType] >>> HOTKEY: onStart blocked – still processing")
                    ...
                    NSLog("[KeyType] >>> HOTKEY: Record START (mode: %@)", effectiveMode.name)
                    ...
                    Task {
                        ...
                        if !ready {
                            NSLog("[KeyType] >>> HOTKEY: previous session did not reach idle in time")
                ...
                onStop: { [weak self] in
                    ...
                    let phase = MainActor.assumeIsolated { self.appState.barPhase }
                    NSLog("[KeyType] >>> HOTKEY: Record STOP (phase=%@)", String(describing: phase))
        ...
        hotkeyManager.onCrossModeStop = { [weak self] newModeId in
            ...
            let effectiveMode = availableModes.first(where: { $0.id == resolvedMode.id }) ?? resolvedMode
            NSLog("[KeyType] >>> HOTKEY: Cross-mode stop → %@", effectiveMode.name)
        ...
        hotkeyManager.onESCAbort = { [weak self] in
            ...
            NSLog("[KeyType] >>> HOTKEY: ESC abort injection (phase=%@)", String(describing: phase))
    ...
    private func startHotkeyWithRetry() {
        ...
        NSLog("[KeyType] Hotkey setup: %@", success ? "OK" : "FAILED (need Accessibility permission)")
    ...
    private func handleHotkeyRetry(_ timer: Timer) {
        if PermissionManager.hasAccessibilityPermission {
            ...
            hotkeyRetryCount += 1
            NSLog("[KeyType] Hotkey retry #%d: %@", hotkeyRetryCount, ok ? "OK" : "still failing")
            if ok {
                timer.invalidate()
                retryTimer = nil
                hotkeyRetryCount = 0
                ...
                retryTimer = nil
                hotkeyRetryCount = 0
                NSLog("[KeyType] Accessibility granted but hotkey tap failed after retries. Suggesting restart.")
                showRestartAlert()
            }
        }
    }
    ...
    private func showRestartAlert() {
        ...
        alert.informativeText = NSLocalizedString(
            "macOS 有时需要重启应用才能激活全局快捷键。点击「重启」自动重启 KeyType。",
            comment: ""
        )
    ...
    private func performMenuBarCheck() {
        ...
        NSLog("[KeyType] Menu bar icon appears hidden by system settings")
        ...
        alert.informativeText = L(
            "macOS 的菜单栏设置可能隐藏了 KeyType 图标。\n\n请前往 系统设置 > 菜单栏，在「允许在菜单栏中显示」列表中开启 KeyType。",
            "macOS may have hidden the KeyType icon.\n\nGo to System Settings > Menu Bar and enable KeyType in the 'Allow in Menu Bar' list."
        )
    ...
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls {
            guard url.scheme == "keytype" else { continue }
            switch url.host {
            case "reload-vocabulary":
                NSLog("[KeyType] URL command: reload-vocabulary")
            ...
            case "auth":
                NSLog("[KeyType] URL command: auth (no-op, code-based auth now)")
            default:
                NSLog("[KeyType] Unknown URL command: \(url)")
            }
        }
    }
...
struct MenuBarContent: View {
    ...
    var body: some View {
        ...
        Button(L("退出 KeyType", "Quit KeyType")) {
