    ...
    func startRecording(mode: ProcessingMode = .direct) async {
        ...
        if provider.isLocal {
            ...
            if let savedConfig = KeychainService.loadASRConfig(for: provider) {
                ...
                SoundFeedback.playError()
                state = .idle
                onASREvent?(.error(NSError(domain: "KeyType", code: -1, userInfo: [NSLocalizedDescriptionKey: L("本地模型未配置", "Local model not configured")])))
                onASREvent?(.completed)
                return
            }
            ...
            if !ModelManager.shared.areRequiredModelsAvailable() {
                ...
                SoundFeedback.playError()
                state = .idle
                onASREvent?(.error(NSError(domain: "KeyType", code: -3, userInfo: [NSLocalizedDescriptionKey: L("请先下载识别模型", "Please download ASR models first")])))
                onASREvent?(.completed)
                return
            }
            ...
            SoundFeedback.playError()
            state = .idle
            onASREvent?(.error(NSError(domain: "KeyType", code: -1, userInfo: [NSLocalizedDescriptionKey: L("未配置 API 凭证", "API credentials not configured")])))
            onASREvent?(.completed)
            return
        }
        ...
        guard let client = ASRProviderRegistry.createClient(for: provider) else {
            ...
            SoundFeedback.playError()
            state = .idle
            onASREvent?(.error(NSError(domain: "KeyType", code: -2, userInfo: [NSLocalizedDescriptionKey: L("\(provider.displayName) 暂不支持", "\(provider.displayName) not yet supported")])))
            onASREvent?(.completed)
            return
        }
        ...
        // Volume lowered in KeyTypeApp .ready handler
...
// via `@testable import KeyType` (see RecognitionSessionTests).
extension String {
