    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            SettingsSectionHeader(
                label: "ABOUT",
                title: L("关于 KeyType", "About KeyType"),
            ...
            HStack(spacing: 12) {
                linkButton("GitHub", icon: "chevron.left.forwardslash.chevron.right") {
                    if let url = URL(string: "https://github.com/Keystone-shiqi/keytype") {
                        NSWorkspace.shared.open(url)
                    }
                }
                linkButton(L("反馈", "Feedback"), icon: "envelope") {
                    if let url = URL(string: "https://github.com/Keystone-shiqi/keytype/issues") {
                        NSWorkspace.shared.open(url)
                    }
                }
            }
    ...
    private var updateSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if appState.availableUpdates.isEmpty && !appState.isCheckingUpdate {
                ...
                Button {
                    if let url = URL(string: "https://github.com/Keystone-shiqi/keytype/releases") {
                        NSWorkspace.shared.open(url)
                    }
                } label: {
    ...
    private var updateActionArea: some View {
        ...
        case .readyToInstall:
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text(L("KeyType 将关闭并自动重启", "KeyType will close and restart automatically"))
                    .font(.system(size: 10))
                    .foregroundStyle(TF.settingsTextTertiary)
            }
