...
final class AppUpdater {
    ...
    private let logger = Logger(subsystem: "com.keytype", category: "AppUpdater")
    ...
    private var stagingDir: URL {
        ...
        return appSupport.appendingPathComponent("KeyType/Updates")
    }
    ...
    private func dmgPath(for version: String, isLocal: Bool) -> URL {
        ...
        return stagingDir.appendingPathComponent("KeyType-v\(version)-\(suffix).dmg")
    }
    ...
    private var updaterUserAgent: String {
        ...
        return "KeyType-Updater/\(version)"
    }
    ...
    private func installTargetURL() -> URL {
        ...
        if currentURL.lastPathComponent == "KeyType-backup.app"
            || path.contains("/KeyType/Updates/")
            || path.contains("/AppTranslocation/")
            || path.hasPrefix("/Volumes/") {
            return URL(fileURLWithPath: "/Applications/KeyType.app")
        }
        return currentURL
    }
    ...
    private func generateUpdaterScript(
        ...
        exec > "$LOG" 2>&1
        echo "KeyType updater started at $(date)"
        ...
        if [ -z "$NEW_APP" ] || [ ! -d "$NEW_APP" ]; then
            echo "ERROR: KeyType.app not found in DMG"
            exit 1
        fi
        echo "Found: $NEW_APP"
        ...
        BACKUP_PATH="$BACKUP_DIR/KeyType.app"
        rm -rf "$BACKUP_DIR"
        mkdir -p "$BACKUP_DIR"
