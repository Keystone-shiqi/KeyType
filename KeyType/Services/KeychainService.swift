...
enum KeychainService {
    ...
    private static let keychainScalarService = "com.keytype.scalar"
    private static let keychainGroupedService = "com.keytype.grouped"
    ...
    /// Merge ~/Library/Application Support/TypeFlow/ files into KeyType/ (one-time, from old project name).
    ...
    private static func migrateAppSupportDirectory() {
        ...
        let newDir = appSupport.appendingPathComponent("KeyType", isDirectory: true)
        ...
        if movedCount > 0 {
            NSLog("[KeychainService] Migrated %d files from TypeFlow → KeyType", movedCount)
        }
