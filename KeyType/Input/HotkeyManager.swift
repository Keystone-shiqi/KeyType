...
final class HotkeyManager: NSObject {
    ...
    private func startHealthCheck() {
        ...
        healthCheckTimer = Timer.scheduledTimer(withTimeInterval: 10.0, repeats: true) { [weak self] _ in
            ...
            if !CFMachPortIsValid(tap) {
                NSLog("[KeyType] Health check: tap port invalid, reinstalling tap...")
                self.reinstallTap()
                return
            }
            ...
            if !CGEvent.tapIsEnabled(tap: tap) {
                NSLog("[KeyType] Health check: tap disabled, re-enabling...")
                CGEvent.tapEnable(tap: tap, enable: true)
                if !CGEvent.tapIsEnabled(tap: tap) {
                    NSLog("[KeyType] Health check: tap re-enable failed, reinstalling tap...")
                    self.reinstallTap()
                }
            }
        }
    ...
    private func reinstallTap() {
        stop()
        let ok = start()
        NSLog("[KeyType] Tap reinstall: %@", ok ? "OK" : "FAILED")
    }
    ...
    private func updateMediaKeySession() {
        ...
        if !isMediaSessionActive {
            ...
            let nowPlayingInfo: [String: Any] = [
                MPMediaItemPropertyTitle: "KeyType Voice Input",
