...
struct CloudSettingsCard: View, SettingsCardHelpers {
    ...
    var body: some View {
        settingsGroupCard(L("KeyType Cloud", "KeyType Cloud"), icon: "cloud.fill") {
            if auth.isLoggedIn {
                loggedInContent
            } else {
                loginContent
    ...
    private var loginContent: some View {
        Text(L(
            "登录后即可使用 KeyType Cloud 语音识别和文本处理服务。免费体验 2000 字。",
            "Sign in to use KeyType Cloud for voice recognition and text processing. 2000 characters free."
        ))
        .font(.system(size: 12))
