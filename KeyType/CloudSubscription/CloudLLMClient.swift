...
enum CloudLLMError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .notAuthenticated: return "Please log in to KeyType Cloud"
...
/// LLM client that proxies requests through the KeyType Cloud backend.
/// The backend handles API keys and upstream provider selection.
actor CloudLLMClient: LLMClient {

    private let logger = Logger(subsystem: "com.keytype.app", category: "CloudLLM")
