#!/usr/bin/env python3
"""DoubaoIme ASR WebSocket server for KeyType.
...
CREDENTIAL_PATH = os.path.expanduser(
    "~/Library/Application Support/KeyType/doubao-asr-credentials.json"
)
...
async def websocket_endpoint(ws: WebSocket):
    ...
    # Audio queue: KeyType sends PCM chunks, we feed them to doubaoime-asr
    ...
    async def receive_audio():
        """Receive PCM audio from KeyType WebSocket client."""
        try:
            while True:
    ...
    async def process_asr():
        """Run ASR and send results back to KeyType."""
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DoubaoIme ASR server for KeyType")
