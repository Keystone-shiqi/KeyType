...
> update：官方[词库管理Skill](https://github.com/Keystone-shiqi/keytype-vocab-skill)，帮你大幅提高识别准确率

<p align="center">
  <img src="docs/images/header-combined.svg" width="100%" alt="KeyType - macOS 语音输入法" />
</p>
...
- **配套Skill**：真正做到100%准确率，打造只属于你的输入法，[点这里安装Skill](https://github.com/Keystone-shiqi/keytype-vocab-skill)后跟你的agent说"Qwen3.5 不要识别成 Queen 3.5"，他就能自动帮你管理热词和映射词，同类错误不再犯 
...
| 版本                                                         | 说明                                                         | 安装包大小   |
| ------------------------------------------------------------ | ------------------------------------------------------------ | ------ |
| ✨推荐：**[云端版本（点击下载）](https://github.com/Keystone-shiqi/keytype/releases/download/v1.9.6/KeyType-v1.9.6-cloud.dmg)** | 支持云端识别 (Intel + Apple Silicon)，需配置语音、大模型API Key。语音识别推荐火山-豆包语音/Soniox、体验最好。火山注册有送额度，单价都十分便宜。[配置指引](https://my.feishu.cn/wiki/QdEnwBMfUi0mN4k3ucMcNYhUnXr) | ~4MB  |
| **[本地版本（点击下载）](https://github.com/Keystone-shiqi/keytype/releases/download/v1.9.6/KeyType-v1.9.6-local-apple-silicon.dmg)** | 内嵌 SenseVoice + Qwen3-ASR 本地识别引擎 (Apple Silicon only，约占用8GB内存，建议32GB以上)，大模型依旧需要配置API Key或Ollama本地服务。 | ~700MB |
...
- **强烈建议**搭配[配套Skill](https://github.com/Keystone-shiqi/keytype-vocab-skill)使用：市面上所有的语音输入法，专有名词均无法做到很好的识别（例如：Qwen 3.5），搭配Skill使用1-2天，你将彻底迈入100%识别准确率
...
| 模块 | 说明 |
|------|------|
| `KeyType/ASR/` | ASR 引擎抽象层，可插拔 Provider 架构 |
| `KeyType/Audio/` | 音频采集 (16kHz mono PCM) |
| `KeyType/Session/` | 核心状态机：录音 → ASR → 注入 |
| `KeyType/Services/` | 凭证存储、热词、模型管理、Python 服务管理 |
| `KeyType/LLM/` | LLM 文本处理 (13 个 provider) |
| `KeyType/Input/` | 全局快捷键管理 |
| `KeyType/Injection/` | 文本注入 (剪贴板 Cmd+V) |
| `KeyType/Bridge/` | SherpaOnnx C API Swift 桥接 (可选) |
| `KeyType/UI/` | SwiftUI 界面：浮窗 + 设置 |
...
> Update: Official [Vocabulary Management Skill](https://github.com/Keystone-shiqi/keytype-vocab-skill) to dramatically improve recognition accuracy

<p align="center">
  <img src="docs/images/header-combined-en.svg" width="100%" alt="KeyType - macOS Voice Input" />
</p>
...
- **Companion Skill**: Achieve 100% recognition accuracy. [Install the Skill](https://github.com/Keystone-shiqi/keytype-vocab-skill) and tell your agent "Don't recognize Qwen3.5 as Queen 3.5" to automatically manage hotwords and snippets. Same mistakes won't happen again.
...
| ------- | ----------- | ---- |
| ✨Recommended: **[Cloud Edition (Download)](https://github.com/Keystone-shiqi/keytype/releases/download/v1.9.6/KeyType-v1.9.6-cloud.dmg)** | Cloud recognition (Intel + Apple Silicon). Requires ASR and LLM API keys. Recommended: Volcano/Doubao or Soniox for best experience. [Setup Guide](https://my.feishu.cn/wiki/QdEnwBMfUi0mN4k3ucMcNYhUnXr) | ~4MB |
| **[Local Edition (Download)](https://github.com/Keystone-shiqi/keytype/releases/download/v1.9.6/KeyType-v1.9.6-local-apple-silicon.dmg)** | Bundled SenseVoice + Qwen3-ASR local recognition (Apple Silicon only, ~8GB RAM, 32GB+ recommended). LLM still requires API key or local Ollama. | ~700MB |
...
>
>   xattr -d com.apple.quarantine /Applications/KeyType.app
>
>   ```bash
...
>
> - Step 3: Open the DMG and drag KeyType to the Applications folder.
>
...
- **Strongly recommended**: Use with the [companion Skill](https://github.com/Keystone-shiqi/keytype-vocab-skill). No voice input tool handles proper nouns perfectly (e.g., "Qwen 3.5"). After 1-2 days with the Skill, you'll achieve 100% recognition accuracy.
...
| Module | Description |
|--------|-------------|
| `KeyType/ASR/` | ASR engine abstraction layer, pluggable provider architecture |
| `KeyType/Audio/` | Audio capture (16kHz mono PCM) |
| `KeyType/Session/` | Core state machine: record > ASR > inject |
| `KeyType/Services/` | Credential storage, hotwords, model management, Python service management |
| `KeyType/LLM/` | LLM text processing (13 providers) |
| `KeyType/Input/` | Global hotkey management |
| `KeyType/Injection/` | Text injection (clipboard Cmd+V) |
| `KeyType/Bridge/` | SherpaOnnx C API Swift bridge (optional) |
| `KeyType/UI/` | SwiftUI interface: floating window + settings |
...
If you're an AI agent (Claude Code, Cursor, Copilot, etc.) tasked with building, deploying, or contributing to KeyType, here's everything you need.
...
```bash
# 1. Clone
git clone https://github.com/Keystone-shiqi/keytype.git && cd keytype
...
- **First deploy** auto-creates a self-signed certificate ("KeyType Local", valid 10 years) if no signing identity exists. This may trigger a **Keychain password prompt** that requires human interaction.
- **Subsequent deploys** reuse the same certificate. Accessibility/Microphone permissions persist, no re-grant needed.
- After first launch, the user must grant **Accessibility permission** once (System Settings > Privacy & Security > Accessibility > enable KeyType).
...
- **Credentials**: stored at `~/Library/Application Support/KeyType/credentials.json` (mode 0600), never in code or environment variables. GUI apps cannot read shell env vars from `~/.zshrc`
...
<a href="https://star-history.com/#Keystone-shiqi/keytype&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=Keystone-shiqi/keytype&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=Keystone-shiqi/keytype&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=Keystone-shiqi/keytype&type=Date" />
 </picture>
</a>
