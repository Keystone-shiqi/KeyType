...
The built binary is at `.build/release/KeyType`. To package it as a `.app` bundle, see `scripts/deploy.sh`.
...
`KeyType/CloudSubscription/marker` file has been renamed to
...
The `KeyType/CloudSubscription/` source directory is preserved as-is for
future reactivation. To re-enable:

```bash
mv KeyType/CloudSubscription/marker.archived-no-subscription \
   KeyType/CloudSubscription/marker
swift package clean
```
...
- `HAS_CLOUD_SUBSCRIPTION`: controlled by `KeyType/CloudSubscription/marker` file presence
...
All subscription/cloud-proxy code lives in `KeyType/CloudSubscription/` (13 files). Main code uses `#if HAS_CLOUD_SUBSCRIPTION` guards at ~11 touch points. When the marker is absent, the directory is excluded from compilation entirely.
...
1. Create a Config file in `KeyType/ASR/Providers/`, implementing `ASRProviderConfig`
...
- Models downloaded from GitHub releases (tar.bz2), stored at `~/Library/Application Support/KeyType/models/`
...
- **Secure fields** (`isSecure: true` in CredentialField, e.g. API keys): stored in macOS Keychain (`com.keytype.grouped` / `com.keytype.scalar` services)
- **Non-secure fields** (model, language, etc.): stored in `~/Library/Application Support/KeyType/credentials.json` (file permissions 0600)
...
| Path | Responsibility |
|---|---|
| `KeyType/ASR/ASRProvider.swift` | Provider enum + protocol + CredentialField |
| `KeyType/ASR/ASRProviderRegistry.swift` | Registry: provider → config + client factory + capabilities |
| `KeyType/ASR/Providers/*.swift` | Per-vendor Config implementations |
| `KeyType/ASR/SpeechRecognizer.swift` | SpeechRecognizer protocol + LLMConfig + event types |
| `KeyType/ASR/SenseVoiceWSClient.swift` | Local ASR client (WebSocket to Python servers, dual-engine) |
| `KeyType/ASR/VolcASRClient.swift` | Cloud streaming ASR (Volcano, WebSocket) |
| `KeyType/ASR/DeepgramASRClient.swift` | Cloud streaming ASR (Deepgram, WebSocket) |
| `KeyType/ASR/ElevenLabsASRClient.swift` | Cloud streaming ASR (ElevenLabs Scribe v2, WebSocket) |
| `KeyType/ASR/OpenAIASRClient.swift` | Cloud batch ASR (OpenAI, REST) |
| `KeyType/ASR/SherpaPunctuationProcessor.swift` | Optional punctuation restoration (SherpaOnnx) |
| `KeyType/Bridge/SherpaOnnxBridge.swift` | SherpaOnnx C API Swift bridge (conditional) |
| `KeyType/Services/SenseVoiceServerManager.swift` | Local Qwen3-ASR Python server lifecycle |
| `KeyType/Session/RecognitionSession.swift` | Core state machine: record → ASR → inject |
| `KeyType/Audio/AudioCaptureEngine.swift` | Audio capture, `getRecordedAudio()` returns full recording |
| `KeyType/UI/AppState.swift` | `ProcessingMode` definition, built-in mode list |
| `KeyType/Services/ModelManager.swift` | SenseVoice model download, validation, selection |
| `KeyType/Services/KeychainService.swift` | Credential read/write (provider groups + migration) |
| `KeyType/Services/HotwordStorage.swift` | ASR hotword storage (UserDefaults) |
| `KeyType/LLM/LLMProvider.swift` | 13 LLM providers (incl. local Qwen offline) |
| `KeyType/LLM/LLMProviderRegistry.swift` | LLM provider → config + client factory |
| `KeyType/Session/SoundFeedback.swift` | Start/stop/error sounds, multiple sound styles |
...
- Bundled WAV files in `KeyType/Resources/Sounds/`, copied to app bundle by deploy.sh
