...
case "$APP_FLAVOR" in
    public)
        APP_NAME="${APP_NAME:-KeyType}"
        APP_BUNDLE_ID="${APP_BUNDLE_ID:-com.keytype.app}"
        URL_SCHEME="${URL_SCHEME:-keytype}"
        ;;
    personal)
        APP_NAME="${APP_NAME:-KeyType CtriXin}"
        APP_BUNDLE_ID="${APP_BUNDLE_ID:-com.ctrixin.keytype}"
        URL_SCHEME="${URL_SCHEME:-keytype-ctrixin}"
        ;;
    *)
...
APP_EXECUTABLE="${APP_EXECUTABLE:-KeyType}"
APP_ICON_NAME="${APP_ICON_NAME:-AppIcon}"
APP_VERSION="${APP_VERSION:-1.9.6}"
APP_BUILD="${APP_BUILD:-1}"
MIN_SYSTEM_VERSION="${MIN_SYSTEM_VERSION:-14.0}"
VARIANT="${VARIANT:-cloud}"    # cloud or local
ARCH="${ARCH:-universal}"      # arm64 or universal
MICROPHONE_USAGE_DESCRIPTION="${MICROPHONE_USAGE_DESCRIPTION:-KeyType 需要访问麦克风以录制语音并将其转换为文本。}"
SPEECH_RECOGNITION_USAGE_DESCRIPTION="${SPEECH_RECOGNITION_USAGE_DESCRIPTION:-KeyType 需要语音识别权限以将你的语音转写为文字。}"
APPLE_EVENTS_USAGE_DESCRIPTION="${APPLE_EVENTS_USAGE_DESCRIPTION:-KeyType 需要辅助功能权限来注入转写文字到其他应用}"
...
elif security find-identity -v -p codesigning 2>/dev/null | grep -q "KeyType Dev"; then
    SIGNING_IDENTITY="KeyType Dev"
else
    SIGNING_IDENTITY="-"
fi
...
if [ "$ARCH" = "arm64" ]; then
    ...
    for candidate in \
        "$PROJECT_DIR/.build/arm64-apple-macosx/release/KeyType" \
        "$PROJECT_DIR/.build/release/KeyType" \
        "$PROJECT_DIR/.build/apple/Products/Release/KeyType"
    do
        if [ -f "$candidate" ]; then
            BINARY="$candidate"
            break
...
if [ -z "$BINARY" ]; then
    BINARY="$(find "$PROJECT_DIR/.build" -path '*/release/KeyType' -type f -not -path '*/x86_64/*' -not -path '*/arm64/*' | head -n 1)"
fi
...
cp "$PROJECT_DIR/KeyType/Resources/${APP_ICON_NAME}.icns" "$APP_PATH/Contents/Resources/${APP_ICON_NAME}.icns" 2>/dev/null || true
...
cp "$PROJECT_DIR/KeyType/Resources/Sounds/"*.wav "$APP_PATH/Contents/Resources/Sounds/" 2>/dev/null || true
...
if [ "$VARIANT" = "local" ]; then
    ...
    SHERPA_SV_MODEL="$HOME/Library/Application Support/KeyType/models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17"
    ...
    SILERO_VAD_MODEL="$HOME/Library/Application Support/KeyType/models/silero_vad"
...
cp "$PROJECT_DIR/KeyType/Resources/THIRD_PARTY_LICENSES.txt" "$APP_PATH/Contents/Resources/" 2>/dev/null || true
