...
APP_PATH="${1:-${APP_PATH:-/Applications/KeyType.app}}"
...
[ -f "$APP_PATH/Contents/MacOS/KeyType" ] || fail "app executable missing"
[ -f "$APP_PATH/Contents/Resources/AppIcon.icns" ] || fail "app icon missing"

[ "$(read_plist CFBundleExecutable)" = "KeyType" ] || fail "CFBundleExecutable should be KeyType"
[ "$(read_plist CFBundleIdentifier)" = "com.keytype.app" ] || fail "CFBundleIdentifier should be com.keytype.app"
[ "$(read_plist CFBundleName)" = "KeyType" ] || fail "CFBundleName should be KeyType"
[ "$(read_plist CFBundleDisplayName)" = "KeyType" ] || fail "CFBundleDisplayName should be KeyType"
