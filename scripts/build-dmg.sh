...
NOTARY_PROFILE="${NOTARY_PROFILE:-keytype-notary}"
...
case "$VARIANT" in
    pure|local) ;;
    official)
        if [ ! -f "$PROJECT_DIR/KeyType/CloudSubscription/marker" ]; then
            echo "ERROR: official variant is archived (subscription paused)."
            echo "       To re-enable, restore KeyType/CloudSubscription/marker first."
            exit 1
        fi
        ;;
    cloud) VARIANT="pure" ;;
...
cleanup() {
    ...
    if [ -f "$PROJECT_DIR/KeyType/CloudSubscription/marker.hidden" ]; then
        mv "$PROJECT_DIR/KeyType/CloudSubscription/marker.hidden" \
           "$PROJECT_DIR/KeyType/CloudSubscription/marker"
    fi
}
trap cleanup EXIT
...
SUB_AVAILABLE="no"
[ -f "$PROJECT_DIR/KeyType/CloudSubscription/marker" ] && SUB_AVAILABLE="yes"
...
if [ "$NEEDS_SUBSCRIPTION" = "0" ] && [ -f "$PROJECT_DIR/KeyType/CloudSubscription/marker" ]; then
    echo "Hiding CloudSubscription for ${VARIANT} build..."
    mv "$PROJECT_DIR/KeyType/CloudSubscription/marker" \
       "$PROJECT_DIR/KeyType/CloudSubscription/marker.hidden"
fi
...
if [ "$SIGNING_IDENTITY" != "-" ]; then
    ...
    while IFS= read -r -d '' file; do
        [ "$file" = "$APP/Contents/MacOS/KeyType" ] && continue
