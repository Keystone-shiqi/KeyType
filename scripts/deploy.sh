...
echo "Stopping KeyType..."
osascript -e "quit app \"$APP_NAME\"" 2>/dev/null || true
sleep 1

APP_PATH="$APP_PATH" bash "$SCRIPT_DIR/package-app.sh"

# Update Keychain partition lists so KeyType can access its stored credentials
...
if [ -n "$NEW_CDHASH" ]; then
    # Collect all KeyType keychain accounts: parse acct+svce pairs from dump.
    ...
    while IFS= read -r line; do
        ...
        /"svce".*"com\.keytype\.(grouped|scalar)"/ { gsub(/.*="/, ""); gsub(/"$/, ""); if(acct) print $0 "|" acct; acct="" }
    ')
    ...
    if [ ${#T4M_ITEMS[@]} -gt 0 ]; then
        ...
        if [ "$NEEDS_UPDATE" = "1" ]; then
            ...
            KC_PASS_FILE="$HOME/.keytype-kc-pass"
            if [ -n "${KC_PASS:-}" ]; then
