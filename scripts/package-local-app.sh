...
APP_PATH="${APP_PATH:-/Applications/KeyType.app}"
APP_BUNDLE_ID="${APP_BUNDLE_ID:-com.keytype.localfixed}"
APP_VERSION="${APP_VERSION:-1.9.4-local}"
APP_BUILD="${APP_BUILD:-2}"
CODESIGN_IDENTITY="${CODESIGN_IDENTITY:-KeyType Local Dev}"
ARCH="${ARCH:-arm64}"
SKIP_QWEN3_BUILD="${SKIP_QWEN3_BUILD:-1}"

DEFAULT_BUNDLED_QWEN3="/Applications/KeyType.app/Contents/Resources/Models/Qwen3-ASR"
...
osascript -e 'quit app "KeyType"' 2>/dev/null || true
pkill -f '/Applications/KeyType.app/Contents/MacOS/KeyType' 2>/dev/null || true
...
HOTWORDS="$HOME/Library/Application Support/KeyType/hotwords.txt"
LOG="${TMPDIR:-/tmp}/keytype-qwen3-smoke.log"
rm -f "$LOG"
...
open -n "$APP_PATH"
echo "Local KeyType app installed at $APP_PATH"
