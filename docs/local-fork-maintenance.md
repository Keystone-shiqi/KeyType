# KeyType Local Fork Maintenance
...
Source: `KeyType/Session/RecognitionSession.swift`
...
```bash
defaults write com.keytype.localfixed tf_enableSpeculativeLLM -bool true
```

To explicitly disable it again:

```bash
defaults write com.keytype.localfixed tf_enableSpeculativeLLM -bool false
```
...
VARIANT=local \
ARCH=arm64 \
APP_BUNDLE_ID=com.keytype.localfixed \
APP_PATH="$HOME/Applications/KeyType Local Fixed.app" \
QWEN3_MODEL_PATH="/Applications/KeyType.app/Contents/Resources/Models/Qwen3-ASR" \
...
```text
~/Applications/KeyType Local Fixed.app
```

After re-signing, reset and grant Accessibility again if hotkeys stop working:

```bash
tccutil reset Accessibility com.keytype.localfixed
open 'x-apple.systempreferences:com.apple.preference.security?Privacy_Accessibility'
```

Grant the fixed app path, not `/Applications/KeyType.app`.
...
ps -axo pid,ppid,pcpu,pmem,rss,etime,args \
  | egrep -i 'KeyType|qwen3-asr|server.py|omlx' \
  | egrep -v 'egrep|Codex'
```
...
```bash
tail -f "$HOME/Library/Application Support/KeyType/debug.log" \
