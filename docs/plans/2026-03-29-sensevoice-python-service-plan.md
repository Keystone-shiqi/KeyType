...
**Architecture:** A Python service (based on pengzhendong/streaming-sensevoice) runs as a subprocess inside KeyType.app, communicating via WebSocket. Swift sends PCM16 audio frames, Python returns JSON transcripts. PyInstaller packages the Python service into a standalone binary.
...
```python
#!/usr/bin/env python3
"""SenseVoice streaming ASR WebSocket server for KeyType."""
...
```bash
cd ~/projects/keytype/sensevoice-server
source .venv/bin/activate
...
echo "KeyType
小悠
SenseVoice" > ~/Library/Application\ Support/KeyType/hotwords.txt
```
...
python server.py \
    ...
    --port 8765 \
    --hotwords-file ~/Library/Application\ Support/KeyType/hotwords.txt \
    --beam-size 3 \
    --context-score 6.0
```
...
**Files:**
- Create: `KeyType/Services/SenseVoiceServerManager.swift`
...
actor SenseVoiceServerManager {
    ...
    private let logger = Logger(subsystem: "com.keytype.sensevoice", category: "ServerManager")
...
Modify `KeyTypeApp.swift` (or wherever app startup is):
...
```bash
cd ~/projects/keytype && swift build 2>&1 | tail -5
```
...
**Files:**
- Create: `KeyType/ASR/SenseVoiceWSClient.swift`
...
actor SenseVoiceWSClient: SpeechRecognizer {
    private let logger = Logger(subsystem: "com.keytype.asr", category: "SenseVoiceWS")
...
```bash
git add KeyType/ASR/SenseVoiceWSClient.swift KeyType/ASR/ASRProviderRegistry.swift
...
**Files:**
- Delete: `KeyType/ASR/SenseVoiceASRClient.swift`
- Modify: `KeyType/Services/ModelManager.swift` - change senseVoiceSmall to not require download (bundled in app)
- Modify: `KeyType/ASR/Providers/SherpaASRConfig.swift` - remove senseVoiceModelDir, vadModelDir

**Step 1: Delete old SenseVoice client**

```bash
rm KeyType/ASR/SenseVoiceASRClient.swift
```
...
**Files:**
- Modify: `KeyType/KeyTypeApp.swift` or app delegate
- Modify: `KeyType/UI/Settings/GeneralSettingsTab.swift`
...
**Files:**
- Create: `KeyType/Resources/THIRD_PARTY_LICENSES.txt`
...
```bash
git add KeyType/Resources/THIRD_PARTY_LICENSES.txt
