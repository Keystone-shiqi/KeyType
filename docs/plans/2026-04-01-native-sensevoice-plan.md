...
**Files:**
- Restore: `KeyType/ASR/SenseVoiceASRClient.swift` (from `git show 4ebd717^:KeyType/ASR/SenseVoiceASRClient.swift`)

**Step 1: Restore file**

```bash
cd ~/projects/keytype
git show 4ebd717^:KeyType/ASR/SenseVoiceASRClient.swift > KeyType/ASR/SenseVoiceASRClient.swift
```
...
2. `~/Library/Application Support/KeyType/models/` (for downloaded models, existing behavior)
...
```bash
git add KeyType/ASR/SenseVoiceASRClient.swift
...
**Files:**
- Modify: `KeyType/ASR/ASRProviderRegistry.swift`
...
```bash
git add KeyType/ASR/ASRProviderRegistry.swift
...
**Files:**
- Modify: `KeyType/Services/SenseVoiceServerManager.swift`
...
```bash
git add KeyType/Services/SenseVoiceServerManager.swift
...
**Files:**
- Modify: `KeyType/ASR/SenseVoiceWSClient.swift`
...
```bash
git add KeyType/ASR/SenseVoiceWSClient.swift
...
- Always bundle sherpa-onnx SenseVoice models (int8 ONNX + tokens.txt) from `~/Library/Application Support/KeyType/models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17/`
- Always bundle Silero VAD model from `~/Library/Application Support/KeyType/models/silero_vad/`
...
let appSupportPath = "~/Library/Application Support/KeyType/models/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-int8-2024-07-17"
```

**Step 3: Verify packaging**

```bash
APP_PATH=/tmp/KeyType-test.app bash scripts/package-app.sh
ls -la /tmp/KeyType-test.app/Contents/Resources/Models/
```
...
```bash
git add scripts/package-app.sh KeyType/ASR/Providers/SherpaASRConfig.swift
...
huggingface-cli download mlx-community/Qwen3-ASR-0.6B-8bit --local-dir ~/Library/Application\ Support/KeyType/models/Qwen3-ASR-0.6B-8bit
```
...
```bash
cd ~/projects/keytype/qwen3-asr-server
source .venv/bin/activate
python server.py --model-path ~/Library/Application\ Support/KeyType/models/Qwen3-ASR-0.6B-8bit --port 19999
...
```bash
du -sh /Applications/KeyType.app
```
