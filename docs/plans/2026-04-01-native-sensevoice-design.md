...
KeyType's local ASR currently runs two Python services:
...
| File | Action | Details |
|------|--------|---------|
| `KeyType/ASR/SenseVoiceASRClient.swift` | **Restore** from git `4ebd717^` | sherpa-onnx offline + Silero VAD |
| `KeyType/ASR/SenseVoiceWSClient.swift` | **Modify** | Remove SenseVoice streaming mode, keep Qwen3 WebSocket connection |
| `KeyType/Services/SenseVoiceServerManager.swift` | **Modify** | Remove SenseVoice Python lifecycle, keep Qwen3 only |
| `KeyType/ASR/ASRProviderRegistry.swift` | **Update** | sherpa provider as default local ASR |
| `KeyType/ASR/Providers/SherpaASRConfig.swift` | **Update** | Model paths point to app bundle |
