...
> 关联 Issue: https://github.com/Keystone-shiqi/keytype/issues/144
>
...
- KeyType Cloud variant（当前 deploy.sh 默认产物）
...
如果本地数据不够，用下面的脚本批量插假数据（跑前先备份 `~/Library/Application Support/KeyType/history.db`）：

```bash
# 备份
cp ~/Library/Application\ Support/KeyType/history.db \
   ~/Library/Application\ Support/KeyType/history.db.bak
...
db = sqlite3.connect(
    "/Users/jonathan/Library/Application Support/KeyType/history.db"
)
cur = db.cursor()
...
# 还原
# mv ~/Library/Application\ Support/KeyType/history.db.bak \
#    ~/Library/Application\ Support/KeyType/history.db
```
...
1. `killall KeyType` 干净启动
2. 打开设置 → 切到「识别历史」tab
3. 不滚动，停留 30 秒

**测量**

- Activity Monitor 记录 30 秒末的 KeyType 内存
...
2. 用诊断模式启动：
   ```bash
   /Applications/KeyType.app/Contents/MacOS/KeyType -LogForEachSlowPath YES &
   ```
3. 打开 Console.app，过滤进程 `KeyType`，搜索 `ForEach` 或 `slow path`
4. 进入「识别历史」tab，滚动列表
...
1. 退出 KeyType，用 Instruments 启动并选 Time Profiler 模板
2. attach 到 KeyType，点录制
...
1. Instruments → Allocations 模板 attach KeyType
